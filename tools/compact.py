...
def main() -> int:
    ...
    old_rows = con.execute(f"select count(*) from read_parquet('{SRC_STR}/*.parquet')").fetchone()[0]
    new_rows = con.execute(f"select count(*) from read_parquet('{DST_STR}/**/*.parquet')").fetchone()[0]
    assert old_rows == new_rows, f"Mất hàng: cũ {old_rows} != mới {new_rows}"

    n_dst = len(list(DST.rglob("*.parquet")))
    print(f"  đích   : {DST}  ({n_dst:,} file, {new_rows:,} rows)")
