...
{% macro priority_reject_reason(col) %}
    case
        ...
        when {{ col }} in ('0', '5', '-1') or try_cast({{ col }} as integer) is not null then 'priority là số nhưng ngoài khoảng 1..4'
        else 'priority là chuỗi lạ không xác định: ' || coalesce({{ col }}, 'NULL')
    end
{% endmacro %}
